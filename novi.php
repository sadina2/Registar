...
$result=mysqli_query($con,"select * from osoba where osobaID='$id'"); 

while($row = mysqli_fetch_array($result))
{
	echo"<h3> Počinitelj prekršaja </h3>";
echo"<br/><b> Ime: </b>" . $row['osobaIme'];
echo"<br/> <b>Prezime: </b>" . $row['osobaPrezime'];
echo"<br/> <b>JMBG: </b>" . $row['jmbg'];
echo"<br/><b>Adresa: </b>" . $row['adresaPrebivalista'];
}
$auto=mysqli_query($con,"select * from automobil where automobilID='$id'"); 
while($row = mysqli_fetch_array($auto))
{

echo"<br/><b> Automobil: </b>" . $row['markaAutomobila'];
echo"<br/> <b>Tablice: </b> " .$row['registarskeTablice'];
}
$pol=mysqli_query($con,"select * from zaposlenik where zaposlenikID='$id'"); 
while($row = mysqli_fetch_array($pol))
{
echo"<h3> Zapisnik sastavio </h3>";
echo"<br/> <b>Funkcija: </b>" . $row['funkcija'];	
echo"<br/><b> Ime: </b> " . $row['zaposlenikIme'];
echo"<br/><b> Prezime: </b> " . $row['zaposlenikPrezime'];


}
$rez=mysqli_query($con,"select * from kazna where kaznaID='$id'"); 
while($row = mysqli_fetch_array($rez))
{
echo"<br/><b> Kazna: </b>" . $row['tipKazne'];
}

$novo=mysqli_query($con,"select * from prekrsaj where ID='$id'"); 
while($row = mysqli_fetch_array($novo))
{
	echo"<br/> <b>Datum prekrsaja: </b>" . $row['datumPrekrsaja'];
}
...
{	
						...
						echo 'alert("ID ne postoji!")';
						echo 'window.location= "izvjestaj.php"';
						echo '</script>';
